...
# 底座坐标系下相机位置参数 单位cm  
x_camera_to_base = 0.0  # 粗调水平中心位置，偏左（即左夹右夹均偏左）调小
y_camera_to_base = 3.7
z_camera_to_base = 24.8  # 粗调垂直中心位置，偏上（即上夹下夹均偏上）调小

# 底座坐标系下乒乓球球心到相机镜头光心的距离 单位cm
D_object_to_camera_upper = 24.5  # 上层的一个乒乓球
D_object_to_camera_lower = 27.5  # 下层的两个乒乓球

# 相机内参矩阵参数
f_x_div_dx = 680.0  # 水平缩放，偏大（即左夹偏左，右夹偏右）调大
f_y_div_dy = 680.0  # 垂直缩放，偏大（即上夹偏上，下夹偏下）调大
u0 = 320.0  # 精调水平中心位置，偏左（即左夹右夹均偏左）调小
v0 = 240.0  # 精调垂直中心位置，偏上（即上夹下夹均偏上）调小
...
tight = 90  # 夹紧时的6号舵机角度(需比读取到的角度略大3~5°)
loose = 30  # 松开使的6号舵机角度
delta_yaw_servo_1 = 0  # 待命位调水平时的1号舵机偏差角度，实际偏左为负值
