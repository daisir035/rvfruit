...
def find_sign(img):
    ...
    """
    flag = 0  # 初始化识别到标识标志位为失败

    img_src = img.copy()  # 保存一份原图
    cv2.imshow("img", img)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)  # bgr转hsv，hsv色彩模式下蓝色特征更明显
    cv2.imshow("hsv", img)
    ...
    for con in contours:  # 遍历所有轮廓
        ...
        if l_t_x > 0 and l_t_y > 0 and r_b_x > 0 and r_b_y > 0 and r_b_y - l_t_y > 0 and r_b_x - l_t_x > 0:
            img = img_src[l_t_y:r_b_y, l_t_x:r_b_x]  # 裁剪矩形区域
            cv2.imshow("sign", img)
            img = cv2.resize(img, (30, 30))  # 缩放至模板尺寸
            img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # 转为灰度图，减小计算量
            cv2.waitKey(1)
            flag = 1  # 标志位置位
...
def detect_turn(cam, sample_time, time_out=8, time_out_en=False):
    ...
    print("\n开始识别转向标识\n")
    t0 = time.time()  # 初始时间
    datalist = []  # 初始化采样结果记录表为空
    num = 0  # 初始化采样次数为0
    majorityElement = 2  # 初始化最佳匹配结果，默认为直行，若未识别到，则直行
...
#            print("turn_current", turn_current)
            datalist.append(turn_current)  # 记录采样结果至记录表
            num += 1
