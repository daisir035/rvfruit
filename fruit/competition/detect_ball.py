...
def find_circle(img, color, circle_number):
    ...
    # 当未检测到目标数量圆，且minRadius参数未达到设置的上限时，循环检测(前一个判断条件可适当改弱，例num >= )
    ...
    while fruit_type == 0:
        ...
        for i in range(1, 5):  # 遍历水果种类，1 2 3 4
            ...
            for j in range(1, n_pic_per_type + 1):  # 遍历这种水果的所有模板
                ...
                sim_data.append(round(max_val, 5))  # 记录当前模板的最大相似度
...
##    cam = 0
#    circle, img = detect_circles(cam, color=yellow, circle_number=2, sample_times=20, time_out=10, time_out_en=False)
...
#    cam = "samples/detect_ball.mp4"
##    cam = 0
#    fruit_type, similarity = detect_type(cam, mode=0, time_out=15, time_out_en=False)
#    print("fruit type:", fruit_mapping(fruit_type))
